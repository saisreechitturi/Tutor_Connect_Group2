...
const createSettingsTable = async () => {
    try {
        ...
        // Create trigger (drop first if exists, then create)
        try {
            await query('DROP TRIGGER IF EXISTS update_settings_updated_at ON settings;');
            await query(`
...
const migrate = async () => {
    try {
        logger.info('[INFO] Settings table migration started');
        await createSettingsTable();
        logger.info('[SUCCESS] Settings table migration completed');
        process.exit(0);
    } catch (error) {
        logger.error('[ERROR] Settings table migration failed:', error);
        process.exit(1);
    }
};
